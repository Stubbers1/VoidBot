...
module.exports = [
	{
		...
		async execute(interaction) {
					...
					components: [
						{
							...
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-00-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-01-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-02-${challenger.id}-${opponent.id}`
						}
					]
				},
				{
					...
					components: [
						{
							...
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-10-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-11-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-12-${challenger.id}-${opponent.id}`
						}
					]
				},
				{
					...
					components: [
						{
							...
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-20-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-21-${challenger.id}-${opponent.id}`
						},
						{
							type: 'BUTTON',
							label: '-',
							style: 'SECONDARY',
							custom_id: `tictactoe-22-${challenger.id}-${opponent.id}`
						}
					]
				}
			];
			...
			if (opponent.id === client.user.id) {
				...
				Object.assign(components[i].components[j], {
					label: 'O',
					style: 'DANGER',
					disabled: true
				});
			...
			await interaction.reply({
				...
				allowedMentions: {
					users: [opponent.id === client.user.id ? challenger.id : opponent.id]
				}
			});
			return true;
		}
	...
	async interaction => {
		...
		if ((challenger_turn ? challengerId : opponentId) !== interaction.user.id)
			return (
				(await interaction.reply({
		...
		if (buttonPressed.label !== '-')
			return (
				(await interaction.reply({
					content: 'You must play in an empty space!',
					ephemeral: true
				})) || true
			);
		...
		if (opponentId === client.user.id) {
			...
			if (options > 0) {
				// this check should never fail because the bot plays first and last
				...
				outerLoop: for (let actionRow of components) {
					for (let button of actionRow.components) {
		...
		if (
			getValue(components[0].components[0].label) +
				getValue(components[1].components[1].label) +
				getValue(components[2].components[2].label) ===
			3
		)
			winner = challengerId;
		if (
		...
		if (
			getValue(components[2].components[0].label) +
				getValue(components[1].components[1].label) +
				getValue(components[0].components[2].label) ===
			3
		)
			winner = challengerId;
		if (
		...
		if (winner !== challengerId && challenger_plays + opponent_plays === 9)
			winner = null; // if all

		let content = '';

		if (winner === undefined) {
			content = `<@${challengerId}> (X) is challenging <@${opponentId}> (O). <@${
				challenger_turn ? challengerId : opponentId
			}> will play next.`;
		} else {
			for (let i = 0; i < 3; i++) {
				for (let j = 0; j < 3; j++) {
			...
			if (winner === null) {
				...
				user_stats.inc(
					winner === challengerId ? opponentId : challengerId,
					'losses'
				);
				content += `<@${winner}> won!`;
			}
		...
		await interaction.update({
			...
			allowedMentions:
				winner === undefined
					? { users: [challenger_turn ? opponentId : challengerId] }
					: { parse: [] }
		});
	},
	{
		...
		async execute(interaction) {
			...
			return (
				(await interaction.reply({
					content: `Tic tac toe stats for <@${user.id}>:
\`\`\`Games played: ${played}
Wins: ${wins}
Losses: ${losses}
Draws: ${draws}\`\`\``,
