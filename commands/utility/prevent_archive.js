module.exports = {
    ...
    const thread = interaction.options.getChannel('thread', true)
		if (!thread.isThread()) return interaction.editReply("The channel specified must be a thread.")
		...
		if (threadIds.includes(thread.id)) {
			...
			try {
				...
				return await interaction.editReply("I couldn't unarchive that channel.")
			}
